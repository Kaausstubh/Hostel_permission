...
const app = express();
// Behind Render / other reverse proxies: correct IPs for rate limiting and logs
...
// CORS: allow local dev ports + configured FRONTEND_URL (with/without trailing slash)
const frontendUrl = (process.env.FRONTEND_URL || '').trim();
const frontendVariants = frontendUrl
  ...
  if (hasQueueInfra()) {
    try {
      const redis = await Promise.race([
        getRedis(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('redis_timeout')), 4000)),
      ]);
      redisReady = Boolean(redis?.isOpen);
    } catch {
      redisReady = false;
    }
  }
